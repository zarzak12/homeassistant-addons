    ...
    while True:
        try:
            async with websockets.connect(WS_URL) as websocket:
                print("✅ Connexion WebSocket établie !", file=sys.stdout)

                async for message in websocket:
                    data = json.loads(message)
                    print(f"📩 Message reçu : {data}", file=sys.stdout)

                    # 🎯 Vérifier si le message contient une URL RTMPS
                    if "rtmps_url" in data:
                        rtmps_url = data["rtmps_url"]
                        print(f"🎥 URL RTMPS détectée : {rtmps_url}", file=sys.stdout)
            ...
            print("🔄 WebSocket déconnecté, tentative de reconnexion dans 5s...", file=sys.stderr)
            await asyncio.sleep(5)
        except Exception as e:
            print(f"❌ Erreur WebSocket : {e}, tentative de reconnexion dans 5s...", file=sys.stderr)
            await asyncio.sleep(5)
